  ...
  $usr = $_SESSION['username'];
	...
	$picturesQuery = $db->query("
      ...
      ON users.username = pictures.username
			where pictures.username = '$usr'

      GROUP BY pictures.id
      ORDER BY num_likes desc, pictures.date desc
  ");
        ...
        <div class = "picture">
          ...
          <br>
		          ...
		          <div class ="col-md-10">
		            ...
		            ?>
		            <?php if(!$number) : ?>
		                <span class="far fa-thumbs-up like fa-3x" data-id="<?php echo $picture->id; ?>"></span>
		                <span class="fas fa-thumbs-up unlike hide fa-3x" data-id="<?php echo $picture->id; ?>"></span>
		            <?php else : ?>
		                <span class="fas fa-thumbs-up unlike fa-3x" data-id="<?php echo $picture->id; ?>"></span>
		                <span class="far fa-thumbs-up like hide fa-3x" data-id="<?php echo $picture->id; ?>"></span>
		            <?php endif; ?>
		            &nbsp;&nbsp;&nbsp;&nbsp;
		              <span class="nush<?php echo $picture->id; ?>" id="nush<?php echo $picture->id; ?>"><?php echo $picture->num_likes; ?> people liked this picture.</span>
									<button style="float:right;vertical-align: bottom;" class="btn-primary btn deletePhoto" onclick="isReply = false;" id="deletePhoto" data-id="<?php echo $picture->id; ?>">Delete Photo</button>
          	</div>
					</div>
          <br>
	...
	$('.deletePhoto').click(function (e) {
      ...
      var id = this_var.data("id");
          ...
          $.ajax({
              ...
              success: function (e1) {
                  if(e1.da = "true"){
										this_var.parent().parent().parent().remove();
									}
									else {
										console.log("error");
									}
